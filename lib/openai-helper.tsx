...
export async function generateActivityRecommendations(userInput: string, tripAdvisorResults: any[]) {
...
2. Real activities from TripAdvisor API with full details including ratings, reviews, photos, and URLs
...
- PRESERVE the following fields from TripAdvisor data: rating, reviewCount, tripAdvisorUrl, image
...
Return a JSON object with this structure:
{
  "activities": [
    {
      "name": "string (from TripAdvisor)",
      "experience": "2-3 sentence description",
      "bestFor": "Why this fits this specific group",
      "cost": "€XX per person or 'Free' or 'Varies'",
      "duration": "X hours or from TripAdvisor data",
      "locationType": "indoor/outdoor/hybrid",
      "activityLevel": "low/moderate/high",
      "specialElement": "What makes it unique",
      "preparation": "What to book/bring",
      "tripAdvisorUrl": "string (REQUIRED - from TripAdvisor)",
      "rating": "number (from TripAdvisor, null if not available)",
      "reviewCount": "number (from TripAdvisor)",
      "image": "string URL (from TripAdvisor, null if not available)"
    }
  ],
  "backupOptions": {
...
CRITICAL: You MUST include rating, reviewCount, tripAdvisorUrl, and image fields from the TripAdvisor data for each activity. Do not omit these fields.
</Output_Format>`
